...
public:
    ...
    bool init(SPI_Handle spiHandle, Semaphore_Handle sem_handle);

    // Sends data via SPI
    void SPI_send(uint8_t count, uint8_t *TxBuf, uint8_t *RxBuf);

    // Powers up the module
    void powerUP(uint8_t mode);

    // Powers down the module
    void powerDown(void);

    // Writes into a given register
    uint8_t writeReg(uint8_t regnumber, uint8_t * data, uint8_t len);

    // Reads data from a given register
    uint8_t readReg(uint8_t regnumber, uint8_t * data, uint8_t len);


    // Checks in the on-device registers if there's data available
    bool isDataAvailable(void);

    // Reads a byte of received data and returns it
